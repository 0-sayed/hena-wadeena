...
function ApplicantRow({
  ...
  return (
    <div className="rounded-lg border border-border/50 p-4 space-y-2">
      ...
      {app.noteAr && <p className="text-sm text-muted-foreground">{t('notePrefixLabel')} {app.noteAr}</p>}
      {showReview && (
        <ReviewForm
          jobId={jobId}
          appId={app.id}
        ...
        <div className="container px-4">
          ...
          {jobs.length === 0 ? (
            ...
            <div className="space-y-3">
              {jobs.map((job) => (
                <JobAccordionRow key={job.id} job={job} posterRatings={posterRatings} appLanguage={appLanguage} t={t} />
              ))}
            </div>
          )}
        </div>
