...
export default function RequestPasswordResetPage() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(error instanceof Error ? error.message : t('resetPasswordRequest.error'));
    } finally {
  ...
  return (
    <Layout title={t('resetPasswordRequest.title')}>
      <PageTransition>
            ...
            <SR>
              <Card className="overflow-hidden rounded-2xl border-border/50 shadow-xl">
                <CardHeader className="pb-2 pt-10 text-center">
                  ...
                  <CardTitle className="text-2xl">{t('resetPasswordRequest.cardTitle')}</CardTitle>
                  <p className="mt-2 text-muted-foreground">
                    {t('resetPasswordRequest.cardSubtitle')}
                  </p>
                </CardHeader>
                <CardContent className="px-8 pb-10 pt-8">
                  <form onSubmit={(event) => void handleSubmit(event)} className="space-y-5">
                    ...
                    >
                      {isSubmitting ? t('resetPasswordRequest.submitting') : t('resetPasswordRequest.submit')}
                    </Button>
                    <p className="text-center text-sm text-muted-foreground">
                      {t('resetPasswordRequest.hasCode')}{' '}
                      <Link to="/password-reset/confirm" className="font-semibold text-primary hover:underline">
                        {t('resetPasswordRequest.confirmLink')}
                      </Link>
                    </p>
                  </form>
                </CardContent>
