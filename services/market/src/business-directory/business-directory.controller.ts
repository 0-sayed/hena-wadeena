...
export class BusinessDirectoryController {
  ...
  @Patch(':id/verify')
  @Roles(UserRole.ADMIN)
  verify(@Param('id') id: string, @Body() dto: VerifyBusinessDto, @CurrentUser() user: JwtPayload) {
    return this.service.verify(id, dto, user.sub);
  }

  @Patch(':id')
  @Roles(UserRole.MERCHANT, UserRole.INVESTOR, UserRole.ADMIN)
  update(@Param('id') id: string, @Body() dto: UpdateBusinessDto, @CurrentUser() user: JwtPayload) {
